...
def when_oprimo_el_boton_de_elminar_proveedor_group1(step, group1):
    btn_eliminar_provee = browser.find_element_by_id(
        'btn_eliminar_proveedor' + group1)
    btn_eliminar_provee.click()
...
def given_que_el_proveedor_group1_se_encuentra_registrado_and_me_encuentro_en_la_pantalla_de_editar(step, group1):
    browser.get('http://localhost:8000/proveedores/4/editar/')
...
def when_cambio_el_nombre_por_group1_and_hago_clic_en_el_boton_de_modificar(step, group1):
    caja_texto_nombre = browser.find_element_by_id('id_organizacion')
...
def then_puedo_ver_el_mensaje_group1(step, group1):
    texto_nuevo_proveedor = browser.find_element_by_tag_name('h1')
    assert texto_nuevo_proveedor.text == group1
...
def then_puedo_ver_mi_numero_de_compra_que_estoy_por_realizar(step):
    numero_compra = browser.find_element_by_tag_name('h1')
    assert numero_compra.text == "Realizar Nueva Compra"
